...
const Card = ({ data }) => {
  const { setSelectedCharacter, setAddFavorite, addFavorite, setOpenMenu } =
    useContext(CharacterPersonContex);
  ...
  const renderIcon = (id) => {
    const isInFavorite =
      addFavorite.filter((item) => item.id === id).length > 0;
  ...
  return (
    ...
    >
      <figure className="image-style">
        <img
          className="w-full rounded-xl size-image"
          src={data.image}
          alt={data.name}
        />
      </figure>
